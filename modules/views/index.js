...
const util = require("@fomobro/default-redis")
...
module.exports = {
    ...
    // },
    displayArrayToChannel: function (array,channel,title)
    {
        return display_array_channel(array,channel,title);
    },
    //
...
let smart_view_creation = async function(input){
    ...
    try{
        ...
        if(input.view && input.msg && input.channel){
            ...
            log.debug(tag,' channcel:  ',input.channel)
            //translate to admin
            output.id = id
            //output.channelId = input.channel
            output.channelId = 'help'
        ...
        output.userId = 'BOT001'
        output.content = json2html(input)
        output.dateAdded = new Date().getTime()
        return output
            ...
            for (let i = 0; i < input.sentences.length; i++) {
                ...
                try{
                    ...
                    if(sentence.response){
                        if(debug) console.log(tag,"response detected ")
                        msg = sentence.response
                        if(debug) console.log(tag,"msg: ",msg)
                        ...
                        for (let j = 0; j < attachmentsSmart.length; j++) {
                            attachments.push(attachmentsSmart[j])
                        }
                    } else {
                        msg = sentence
                        let attachmentsSmart = create_view_smart(sentence)
                        if(debug) console.log(tag,"attachment: ",attachmentsSmart)
                        for (let j = 0; j < attachmentsSmart.length; j++) {
                            attachments.push(attachmentsSmart[j])
                        }
                    }
...
const display_array_channel = async function(array,channel,title) {
    ...
    try {
        ...
        // message.view = params
        // message.msg = title
        // let channelRedis = "publish"
        // publisher.publish(channelRedis,JSON.stringify(message))
...
const display_string_channel = async function(string,channel) {
    ...
    try {
        ...
        if(channel) channel = channel.trim()
        if(debug) console.log(tag,"attachments: ",attachments)
        if(debug) console.log(tag,"channel: ",channel)
        if(debug) console.log(tag,"string: ",string)
