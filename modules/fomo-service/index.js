...
let URL_BASE  = process.env.VUE_APP_SERVICE_HOSTNAME || 'http://fomobro.com';
...
let lastPrice = async function (asset) {
    try {
        let output;

        let url = URL_BASE + '/lastPrice/'+asset;
        let result = await send_get_request(url);
...
let getPrediction = async function (id) {
    let tag = " | getPredictions | ";
    try {
        let output;

        let url = URL_BASE + '/predictions/'+id;
