...
module.exports = function (s, conf) {
    ...
    function executeSignal(signal, _cb, size, is_reorder, is_taker, reverseCalled) {
        log.info("Checkpoint executeSignal")
    ...
    function onTrade(trade, is_preroll, cb) {
        ...
        var mins = moment.duration(moment(now()).diff(s.period.last_try_trade)).asMinutes()
        if (trade.time > s.period.close_time || (!s.in_preroll && so.mode != 'sim' && mins >= so.interval_trade)) {
